...
"""
Codes are implemented by modifying the next references for our proposed methods
[1] Wang Y, Liu Y, Zhao S, et al. CAMixerSR: Only Details Need More ``Attention". 
IEEE Transactions on Wireless Communications, 2023.
[2] Choi, H., & Seo, D. Feature Importance-Aware Deep Joint Source-Channel Coding for Computationally Efficient and Adjustable Image Transmission. arXiv preprint arXiv:2504.04758, 2025.
"""
...
class ROIPredictorLG(nn.Module):
    ...
    def forward(self, input_x, mask=None, ratio=0.5, train_mode=False):
        ...
        pred_score = mask #pred_score.size() = B X N
...
class ROIMixer(nn.Module):
    def __init__(self, dim, window_size=8, bias=True, is_deformable=True, ratio=0.5):
        ...
        k = 3
        d = 2
        self.mask_feature_refine = nn.Conv2d(8, dim, 1, 1, 0, bias = bias)
        ...
        self.act = nn.GELU()
        # Predictor
        self.route = ROIPredictorLG(dim,window_size,ratio=ratio)

    def forward(self,x,condition_global=None, mask=None, mask_feature=None, train_mode=False):
        N,C,H,W = x.shape

        mf = self.mask_feature_refine(mask_feature)
        x = x + mf # combine x with mask attention information
        v = self.project_v(x)
        ...
        mask, offsets, ca, sa = self.route(_condition,ratio=self.ratio,mask=mask,train_mode=train_mode) #mask.size() = B X N X 1, N=(h w)
...
class ROIBlock(nn.Module):
    def __init__(self, n_feats, window_size=8, ratio=0.5):
        ...
        self.mixer = ROIMixer(n_feats,window_size=window_size,ratio=ratio)
        self.norm2 = LayerNorm(n_feats)
        self.ffn = GatedFeedForward(n_feats)
        
        
    def forward(self,x,condition_global=None,mask=None,mask_feature=None):
        res = self.mixer(x,condition_global,mask=mask,mask_feature=mask_feature)
        x = self.norm1(x+res)
        res = self.ffn(x)
        x = self.norm2(x+res)
        return x 
...
class ROIGroup(nn.Module):
    def __init__(self, n_feats, n_block, window_size=8, ratio=0.5):
        ...
        self.mask_embedding = nn.Sequential(nn.Conv2d(1, 8, 1, 1, 0, bias=True),
                                        nn.LeakyReLU(negative_slope=0.1, inplace=True),
                                        nn.Conv2d(8, 8, 5, 1, 2, bias=True),
                                        nn.LeakyReLU(negative_slope=0.1, inplace=True))

        self.body = nn.ModuleList([ROIBlock(n_feats, window_size=window_size, ratio=ratio) for i in range(n_block)])
        self.body_tail = nn.Conv2d(n_feats, n_feats, 1, 1, 0)
        
    def forward(self,x,mask):
        condition_global = self.global_predictor(x)
        N,C,H,W = x.shape
        mask = spatial_mask_fit(mask,H,W) #from common_component, mask.size = B X H X W
        mask = mask.unsqueeze(dim=1)
        mask_feature = self.mask_embedding(mask)
        mask = mask.squeeze(dim=1)
        mask = spatial_mask_fit(mask,int(H//self.window_size),int(W//self.window_size))
        mask = rearrange(mask,'b h w -> b (h w)')
       
        shortcut = x.clone()

        for _, blk in enumerate(self.body):
            x = blk(x,condition_global,mask=mask,mask_feature=mask_feature)
